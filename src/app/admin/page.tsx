...
import { getPlatformStats, getAuditLog } from "@/lib/store";
...
function AdminOverview() {
  ...
  const [recentUsers, setRecentUsers] = useState(getUsers().slice(-5).reverse());
  const [audit, setAudit] = useState(getAuditLog().slice(0, 8));

  useEffect(() => {
    setStats(getPlatformStats());
    setRecentUsers(getUsers().slice(-5).reverse());
    setAudit(getAuditLog().slice(0, 8));
  }, []);
    ...
    <div>
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-white/10 bg-white/[0.03] p-6">
          ...
          <div className="mt-4 space-y-3">
            {recentUsers.map((u) => (
              <div key={u.id} className="flex items-center justify-between border-b border-white/5 pb-3 last:border-0">
                <div>
      ...
      <div className="mt-8 rounded-xl border border-white/10 bg-white/[0.03] p-6">
        ...
        <div className="mt-4 space-y-2">
          ...
          {audit.length === 0 && <p className="text-sm text-white/40">No activity yet.</p>}
        </div>
      </div>
    </div>
  );
